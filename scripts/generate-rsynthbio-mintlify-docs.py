...
def vignette_pages() -> list[dict[str, object]]:
    return [
        {
...
def regenerate_docs(source_root: Path, output_dir: Path) -> None:
    global ROOT, OUTPUT_DIR
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate Mintlify docs for rsynthbio from package sources.")
    parser.add_argument("source_root", type=Path, help="Path to the rsynthbio checkout")
    parser.add_argument("output_dir", type=Path, help="Directory where generated Mintlify docs should be written")
    return parser.parse_args()


if __name__ == "__main__":
    args = parse_args()
    regenerate_docs(args.source_root, args.output_dir)
